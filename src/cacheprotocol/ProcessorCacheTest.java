...
public class ProcessorCacheTest {
	...
	int retirementPolicy = Integer.parseInt(args[5]);
	
	/*Max tag is used as this is a parameter used when calculating access statistics
	 * This limits the maximum address to 17407.
	*/
	int maxTag = 34;
	String tracefile = args[6];
	ProcessorCache[] ListOfSeparateProcesses = new ProcessorCache[noOfProcessors];
	int[][][] accessToCacheLine = new int[4][cacheLines][maxTag];
	int[][][] accessToCacheLineRead = new int[4][cacheLines][maxTag];
	int[][][] accessToCacheLineWrite = new int[4][cacheLines][maxTag];
	
	


	for (int i = 0; i < ListOfSeparateProcesses.length; i++) {
	    ListOfSeparateProcesses[i] = new ProcessorCache(cacheLines, lineSize, i, enablePrinting, noOfProcessors, bufferSize, retirementPolicy);
	}
	...
	for (int i = 0; i < ListOfSeparateProcesses.length; i++) {
	    ListOfSeparateProcesses[i].emptyStoreBuffer();
	}
